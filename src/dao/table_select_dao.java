...
public class table_select_dao {
	// ��ѯ��������
	@Test
	public List<user_insert> select(String start_time, String end_time, String hostname) {
		///
		jdbcTem_util jdbcTem_util = new jdbcTem_util();
		JdbcTemplate jdbcTemplate = jdbcTem_util.getJdbcTemplate();
		String sql = "select * from basicinfo  where wcsj BETWEEN ? and ? and hostname=?";
		System.out.println(start_time);
		System.out.println(end_time);
		List<user_insert> users = jdbcTemplate.query(sql, new myRowMapper0(), start_time, end_time, hostname);
